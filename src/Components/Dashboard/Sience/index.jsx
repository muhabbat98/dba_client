...
export default function Sience() {
    ...
    useEffect(() => {
        ...
        else if (data && data.createScience.status===200){
            setOpen(true)}
        else if(data && data.createScience.status!==200) alert(data.createScience.error)
    }, [error, data])
    const sendScience = () => {
        if (enterence.title) {
            scienceBook({
    ...
    return (<>
        ...
        <Snackbar
            ...
            onClose={handleClose}
            message={data && data.createScience.status===200 && data.createScience.data}
            action={
                <React.Fragment>
